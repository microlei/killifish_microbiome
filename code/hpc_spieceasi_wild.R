...
ps_gut_abund_NBH <- ps %>% subset_samples(fishType=="New Bedford Harbor wild") %>% prune_taxa(taxa_sums(.)>50,.)
...
ps_gut_abund_SC <- ps %>% subset_samples(fishType=="Scorton Creek wild") %>% prune_taxa(taxa_sums(.)>50,.)
...
saveRDS(res_NBH, file="../data/processed/spieceasi_wild_NBH.rds")
saveRDS(res_SC, file="../data/processed/spieceasi_wild_SC.rds")
