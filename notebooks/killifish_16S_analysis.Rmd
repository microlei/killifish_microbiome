...
wild_net <- tax_dist %>% as.matrix() %>% graph.adjacency("undirected",weighted=TRUE)
#wild_net <- make_network(ps_wild_abund, "taxa", distance=tax_dist, keep.isolates = TRUE, max.dist = 1)
...
V(wild_net)$taxname <- wild_otudata[,"taxname"]
V(wild_net)$Order <- wild_otudata[,"Order"]
...
ggplot(netg, aes(x = x, y = y, xend = xend, yend = yend)) +
  geom_edges(aes(alpha = weight, size=weight)) + scale_size(range=c(0,3)) + scale_alpha(range=c(0,.5)) +
  geom_nodes(aes(color=Order)) + 
  geom_nodelabel_repel(aes(label=taxname, fill=Order))+
  theme_blank()
```
